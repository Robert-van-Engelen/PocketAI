' ALCATRAZ!
' A self-learning strategy game by Robert van Engelen
'
' For PC-1350/2500: set O=91 at line 10
' For PC-1360:      set O=19 at line 10

' DEF-A or RUN to start
' DEF-C to continue after BRK

' VARIABLES
'  O              O=91 for PC-1350/2500 and O=19 for PC-1360
...
'  M$(65,19)*1    prisoner's memory 1320 addresses of one byte or *2 two bytes for PC-1350
...
10 "A" CLEAR: O=19: CLS: WAIT 0: RANDOM: RESTORE: GOSUB 900
20 "C" GOSUB 700: IF RND 2=1 GOSUB 300
30 GOSUB 200: GOSUB 300: IF P>0 IF M<30 GOTO 30
40 IF P=0 OR M=30 CURSOR O: PRINT "I win": E=E+1
50 IF M=30 CURSOR 43: PRINT "TIMED": CURSOR 67: PRINT "OUT"
...
120 K$=INKEY$
130 GCURSOR (P(G(N))+6,Q(G(N))+7): GPRINT A$(4+(A AND 1)): A=A+1,N=N+1: IF N=3 LET N=0
140 K$=K$+INKEY$: IF K$="" GOTO 110
150 K=ASC K$: IF K<48 OR K>58 LET K=58
160 RETURN

' move guard, determine F=from room (0 to 10), T=to room (0 to 10), K=key press, H=guard moved (0,1,2)
200 CURSOR 91: PRINT "?    ": IF O=19 CURSOR 43: PRINT "move": CURSOR 67: PRINT "guard"
210 GOSUB 100: F=K-48,H=(G(0)=F)+2*(G(1)=F)+3*(G(2)=F)-1: IF H<0 BEEP 1: GOTO 210
220 CURSOR 91: PRINT CHR$ K;"->?"
...
260 NEXT I: GOTO 230
270 CURSOR 94: PRINT CHR$ K: X=P(F),Y=Q(F),G(H)=T: LINE (X+6,Y+1)-(X+9,Y+6),R,BF
...
340 NEXT I
350 CURSOR 91: PRINT L$: IF O=19 CURSOR 19: PRINT L$: CURSOR 43: PRINT L$: CURSOR 67: PRINT L$
' the prisoner can still move to another room when F>0 but not if F=0 (no freedom)
360 IF F GOTO 500
370 CURSOR 19: PRINT "JAIL": CURSOR 43: PRINT "TIME": CURSOR 68: PRINT "you": CURSOR 92: PRINT "win": W=W+1,P=-1
...
390 S=S(I),J=INT(S/20),K=S-20*J
' set memory M$() bit to 1 to mark off the room choice
400 B=2^ASC R$(I) OR ASC M$(J,K),M$(J,K)=CHR$ B
...
' S=S+NCR(10-I,3-J)
540 X=10-I,Y=3-J,Z=X-Y,Z=(Z<Y)*Z+(Z>=Y)*Y,Y=1,U=(Z=0)*X+1
550 FOR Z=X-Z+1 TO X: Y=Y*Z/U,U=U+1: NEXT Z
560 S=S+Y
...
600 J=INT(S/20),K=S-20*J,B=(ASC M$(J,K) OR 255*2^F) AND 255,M$(J,K)=CHR$ B
...
630 IF J=8 CURSOR O: PRINT "oh no": LET J=RND F-1: GOTO 650
' prisoner avoided a trap
640 IF J CURSOR O: PRINT "ha ha"
...
910 FOR I=0 TO 10: READ P(I),Q(I),D$(I),C$(I): CURSOR I: PRINT " * Alcatraz! *": NEXT I
...
950 DIM G(2),M$(65,19)*(1+(O=91)),R$(29)*1,S(29),V(7): E=0,W=0,L$="     "
960 WAIT 200: GPRINT
970 RETURN
